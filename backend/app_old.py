...
def dashboard_stats():
    ...
    medium_priority = len([c for c in candidates if c['priority'] == 'Medium'])
    low_priority = len([c for c in candidates if c['priority'] == 'Low'])
    ...
    return jsonify({
        ...
        'medium_priority': medium_priority,
        'low_priority': low_priority,
        'recent_applications': candidates[-5:] if candidates else []
    }), 200
...
def schedule_interview():
    ...
    interview_slots = data.get('slots')  # Array of datetime slots
    interviewers = data.get('interviewers')  # Array of interviewer names
    ...
    interview = {
        'id': f"INT{len(candidates) + 1}",
        'candidate_id': candidate_id,
        'candidate_name': candidate['name'],
        'job_id': candidate['job_id'],
        'proposed_slots': interview_slots,
...
def submit_feedback():
    ...
    scores = data.get('scores')  # Dict of skill: score
    recommendation = data.get('recommendation')  # Hire/No Hire
    ...
    if 'feedbacks' not in candidate:
        candidate['feedbacks'] = []
    
    candidate['feedbacks'].append({
        'feedback': feedback,
        'scores': scores,
    ...
    for fb in candidate['feedbacks']:
        all_scores.extend(fb['scores'].values())
    
    avg_score = sum(all_scores) / len(all_scores) if all_scores else 0
    candidate['interview_score'] = round(avg_score, 2)
    
    # Update status based on recommendations
    recommendations = [fb['recommendation'] for fb in candidate['feedbacks']]
    if all(r == 'Hire' for r in recommendations):
        candidate['status'] = 'Ready for Offer'
    elif any(r == 'No Hire' for r in recommendations):
...
def generate_offer():
    ...
    job = next((j for j in jobs if j['id'] == candidate['job_id']), None)
    
    # Create offer
    offer = {
        'id': f"OFFER{len(candidates) + 1}",
        'candidate_id': candidate_id,
        'candidate_name': candidate['name'],
...
@app.route('/api/offer/<candidate_id>/accept', methods=['POST'])
def accept_offer(candidate_id):
    """Candidate accepts offer"""
    candidate = next((c for c in candidates if c['id'] == candidate_id), None)
    if not candidate or 'offer' not in candidate:
        return jsonify({'error': 'Offer not found'}), 404
    
    candidate['offer']['status'] = 'Accepted'
    candidate['status'] = 'Offer Accepted'
    candidate['offer']['accepted_at'] = datetime.now().isoformat()
    save_json(CANDIDATES_FILE, candidates)
...
@app.route('/api/engagement/<candidate_id>', methods=['GET'])
def track_engagement(candidate_id):
    """Track candidate engagement (mock data for demo)"""
    candidate = next((c for c in candidates if c['id'] == candidate_id), None)
    if not candidate:
        return jsonify({'error': 'Candidate not found'}), 404
    ...
    engagement = {
        'candidate_id': candidate_id,
        'email_open_rate': random.randint(60, 100),
        'portal_logins': random.randint(2, 10),
...
def get_candidate_score(candidate_id):
    """Score and categorize candidate"""
    candidate = next((c for c in candidates if c['id'] == candidate_id), None)
    if not candidate:
        return jsonify({'error': 'Candidate not found'}), 404
    ...
    score_data = candidate_scorer.calculate_score(candidate, job_requirements)
    
    # Save score to candidate record
    candidate['score'] = score_data['overall_score']
    candidate['category'] = score_data['category']
    candidate['hire_probability'] = score_data['hire_probability']
    candidate['skill_gaps'] = score_data['skill_gaps']
    save_json(CANDIDATES_FILE, candidates)
    
    # Send automated email based on score
    if score_data['overall_score'] >= 75:
        email_template = f"""
        <h2>Congratulations! You've Passed Our Screening</h2>
        <p>Hi {candidate.get('name')},</p>
        <p>We reviewed your application for <strong>{job.get('title')}</strong> and are impressed with your profile.</p>
        <p>Next Step: You'll receive an assessment link shortly. Complete it within 7 days.</p>
        <p><a href="http://localhost:3000">View Your Dashboard</a></p>
        """
        integration_manager.send_notification(candidate, 'screening_passed', email_template)
    elif score_data['overall_score'] >= 50:
        email_template = f"""
        <h2>Application Status Update</h2>
        <p>Hi {candidate.get('name')},</p>
        <p>Your application is under review. We'll get back to you within 2 days.</p>
        <p>Skills matched: {', '.join(candidate.get('skills', [])[:3])}</p>
        """
        integration_manager.send_notification(candidate, 'screening_under_review', email_template)
    else:
        email_template = f"""
        <h2>Application Status Update</h2>
        <p>Hi {candidate.get('name')},</p>
        <p>Thank you for your interest. We found roles that might be a better fit for your profile.</p>
        <p><a href="http://localhost:3000">Explore Other Opportunities</a></p>
        """
        integration_manager.send_notification(candidate, 'screening_failed', email_template)
...
def get_salary_recommendation(job_id):
    ...
    experience = int(job.get('experience_required', '3-5').split('-')[0])
...
def assign_assessment(candidate_id):
    ...
    <p>Complete by: {datetime.now() + __import__('datetime').timedelta(days=7)}</p>
    """
...
def schedule_interview(candidate_id):
    ...
    <p>Hi {candidate.get('name')},</p>
    <p>Great news! Your interview is scheduled for <strong>{available_slots[0]['start']}</strong></p>
    <p><strong>Interview Details:</strong></p>
    <ul>
        <li>Round 1: Technical (1 hour) - {panel_recommendation['recommended_panel'][0].get('name')}</li>
        <li>Round 2: System Design (1 hour) - {panel_recommendation['recommended_panel'][1].get('name') if len(panel_recommendation['recommended_panel']) > 1 else 'TBD'}</li>
        <li>Round 3: Cultural Fit (30 min) - {panel_recommendation['recommended_panel'][2].get('name') if len(panel_recommendation['recommended_panel']) > 2 else 'TBD'}</li>
    </ul>
...
def generate_offer(candidate_id):
    ...
    salary_rec = salary_recommender.get_salary_recommendation(
        job.get('title'),
        job.get('location'),
        int(job.get('experience_required', '3-5').split('-')[0])
    )
